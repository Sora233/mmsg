...
func (i *ImageBytesElement) PackToElement(client *client.QQClient, target Target) message.IMessageElement {
	...
	case TargetPrivate:
		if i.Buf != nil {
			...
			logrus.Errorf("TargetPrivate %v UploadGroupImage error %v", target.TargetCode(), err)
		} else {
			logrus.Debugf("TargetPrivate %v nil image buf", target.TargetCode())
		}
	case TargetGroup:
		if i.Buf != nil {
			...
			logrus.Errorf("TargetGroup %v UploadGroupImage error %v", target.TargetCode(), err)
		} else {
			logrus.Debugf("TargetGroup %v nil image buf", target.TargetCode())
		}
	default:
